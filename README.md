...
| プレイング | 保存したデッキを選び「回し方」と現環境ローテ・メタデッキ（Tier1〜2）とのマッチアップを記録。各メタは狙い・キーカード・警戒点が下書き済み。「自動下書き」で自デッキの回答札を提案。連携進化ロイヤル/ランプドラゴン/スペルウィッチは参考構築(40枚)をワンクリック取込。対戦記録があれば実測勝率チップを併記 |
| アドバイザー | 自分のデッキ・先攻/後攻・相手デッキ・ターン・盤面状況（自分の体力/相手の攻撃力/除去必須）を入力→ ①マリガンのキープ推奨度%＋手札診断 ②今ターンの推奨アクション（PP内の最適プレイを候補%・進化(EP)推奨・リーサル圏警告・同名複数引き対応）③相手の警戒カードを超幾何分布で確率表示。**効果解析込み**（エンハンス/連携/ネクロマンス/直接召喚/PP加速/進化タイミングを効果テキストから検出、対面別の必要な回答、カーブ衝突、相手盤面を加味）。**%は勝率でなくヒューリスティック/確率推定** |
...
| 対戦記録 | 実戦の勝敗（自デッキ×相手メタ×先後×メモ）を記録→マッチアップ別の実測勝率を自動集計。プレイング/アドバイザーに実績を併記し、推定を自分のデータで検証できる |
...
デッキ・知識（プレイング・対戦記録・コンボ・マッチアップ・ターン別）はブラウザの localStorage に保存されます。
