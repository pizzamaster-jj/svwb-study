...
| アドバイザー | 自分のデッキ・先攻/後攻・相手デッキ・ターンを入力→ ①マリガンのキープ推奨度%＋手札診断 ②今ターンの推奨アクション（PP内の最適プレイを候補%で提示）③相手の警戒カードを超幾何分布で確率表示。**効果解析込み**（エンハンス/連携/ネクロマンス/直接召喚/PP加速/進化タイミングを効果テキストから検出、対面別の必要な回答、カーブ衝突を加味）。**%は勝率でなくヒューリスティック/確率推定** |
